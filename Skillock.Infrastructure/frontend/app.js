...
function initAuthView() {
    ...
    document.getElementById('registerForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (error.status === 409) {
                showError('registerError', 'El usuario o email ya existe');
            } else {
    ...
    if (joinBetForm) {
        joinBetForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                showError('joinBetError', 'No se pudo unir a la apuesta');
            }
        });
    }
...
async function joinBetByCode(betCode) {
    try {
        ...
        showError('loginError', 'No se pudo unir a la apuesta');
    }
}
...
async function viewBetDetails(betId) {
    try {
        ...
        showError('counterMontoError', 'Error al abrir la apuesta');
    }
}
...
function initWalletView() {
    ...
    document.getElementById('depositForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showError('depositError', 'Error al realizar el depósito');
        }
    });
    ...
    document.getElementById('withdrawalForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (error.status === 422) {
                showError('withdrawalError', 'Saldo insuficiente');
            } else {
                showError('withdrawalError', 'Error al realizar el retiro');
            }
        }
    });
...
function initLinkGameAccountModal() {
    document.getElementById('linkGameAccountForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (error.status === 409) {
                showError('linkGameAccountError', 'La cuenta ya existe');
            } else {
                showError('linkGameAccountError', 'Error al vincular la cuenta');
            }
        }
    });
}
...
function initNewBetModal() {
    document.getElementById('newBetForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showError('newBetError', 'Error al crear la apuesta');
        }
    });
}
...
async function loadNegotiationData(betCode) {
    try {
        ...
        showError('counterMontoError', 'Error al cargar la apuesta');
    }
}
...
function initNegotiationView() {
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                showNegotiationMessage('error', 'No se pudo copiar el código');
            }
        });
    }

    // Botón aceptar monto
    document.getElementById('acceptMontoBtn').addEventListener('click', async () => {
        if (!currentBetNegotiation) return;
        try {
            const result = await apiRequest(`/bets/${currentBetNegotiation.id}/monto/confirmacion`, 'PUT', {});
            if (result) {
                showNegotiationMessage('success', 'Monto confirmado exitosamente');
                setTimeout(() => loadNegotiationData(currentBetNegotiation.id), 500);
            }
        } catch (error) {
            showNegotiationMessage('error', 'Error al confirmar el monto');
        }
    });
    ...
    document.getElementById('counterMontoBtn').addEventListener('click', async () => {
        ...
        try {
            const result = await apiRequest(`/bets/${currentBetNegotiation.id}/monto`, 'PUT', {
                montoPerTeam: monto
            });
            ...
            showNegotiationMessage('error', 'Error al proponer el monto');
        }
    });
    ...
    document.getElementById('cancelBetBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            showNegotiationMessage('error', 'Error al cancelar la apuesta');
        }
    });
}
